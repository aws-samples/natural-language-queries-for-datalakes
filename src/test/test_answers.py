...
log_file_name = f"{python_script_name}.txt"
...
def runQuery(query, type):
    ...
    db_name = "Chinook.db"
    dir = "/Users/winmaxim/git/datagenie/winmaxim/natural-language-queries-for-datalakes/src/example_data/datalake/data"
...
def main(question_groups, start_with=0, run_up_to=None, run_only_group=None):
    suffix = ""
    if start_with:
    ...
    for question_group in question_groups:
        group_count += 1
        (question_count, question_group_no, subquestion_no, question, expected_sql) = (question_group["QUESTION_NO"], question_group["GROUP_NO"], question_group["SUBQUESTION_NO"], question_group["QUESTION"], question_group["ANSWER"])
        ...
        data_text = runQuery(query, "GENERATED")
        data_text = str(data_text).strip()
        data_text_trimmed = "FIRST 10 ROWS:\n" + "\n".join(data_text.split("\n")[:10]) 

        expected_data_text = runQuery(expected_sql, "EXPECTED")
...
if __name__ == "__main__":
    questions = []
    with open('/Users/winmaxim/git/datagenie/winmaxim/natural-language-queries-for-datalakes/src/test/Chinook_tests.json') as f:
        questions = json.load(f)

    question_groups = []
    for question_no, question in enumerate(questions):
        question_groups.append({"QUESTION_NO": question_no+1, "GROUP_NO": question_no+1, "SUBQUESTION_NO": 0, "QUESTION": question["question"], "ANSWER": question["sql_query"]})

    print(f"RUNNING {question_groups}")

    main(question_groups, start_with=0, run_up_to=None, run_only_group=None)
